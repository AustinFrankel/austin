...
export function DisplayControls() {
  ...
  return (
    <div className="flex items-center gap-3 shrink-0 whitespace-nowrap">
      <div className="flex items-center gap-1 rounded-full border border-black/10 dark:border-white/20 px-2 py-1 shrink-0">
        {accents.map((a) => (
          <button
            key={a.id}
      ...
      <div className="flex items-center rounded-full border border-black/10 dark:border-white/20 overflow-visible shrink-0">
      ...
      <button aria-label="Toggle theme" className="h-8 px-3 rounded-full border border-black/10 dark:border-white/20" onClick={() => setTheme(theme === "light" ? "dark" : "light")}>{theme === "light" ? "Dark" : "Light"}</button>
    </div>
  );
} 